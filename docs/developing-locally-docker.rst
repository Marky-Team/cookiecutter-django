...
1. On the ``nginx-proxy`` service in ``docker-compose.yml``, change ``depends_on`` to ``node`` instead of ``django``.

2. On the ``node`` service in ``docker-compose.yml``, add the following environment configuration:
