...
docker compose -f docker-compose.yml run django python manage.py makemigrations --check || { echo "ERROR: there were changes in the models, but migration listed above have not been created and are not saved in version control"; exit 1; }
...
docker compose -f docker-compose.yml run \
  ...
  -e MAILGUN_API_KEY=x \
  -e MAILGUN_DOMAIN=x \
  django python manage.py check --settings=config.settings.combined --deploy --database default --fail-level WARNING
